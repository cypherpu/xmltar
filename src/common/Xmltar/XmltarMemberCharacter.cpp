...
std::string XmltarMemberCharacter::CompressedMemberHeader(){
	return options_.archiveMemberCompression_.get()->CompressString(MemberHeader());
}

std::string XmltarMemberCharacter::CompressedMemberTrailer(){
	return options_.archiveMemberCompression_.get()->CompressString(MemberTrailer());
}

size_t XmltarMemberCharacter::NumberOfFileBytesThatCanBeArchived(size_t committedBytes, size_t pendingBytes, std::shared_ptr<Transform> archiveCompression){
...
bool XmltarMemberCharacter::IsComplete(){
	return nextByte_==file_size;
}

boost::filesystem::path XmltarMemberCharacter::filepath(){
	return filepath_;
}

size_t XmltarMemberCharacter::NextByte(){
	return nextByte_;
}

void XmltarMemberCharacter::RecalculateMemberHeader(){
